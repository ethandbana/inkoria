...
import { Settings, Grid3X3, BookOpen, Heart, LogOut, Bookmark } from "lucide-react";
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // Liked books
      const { data: likes } = await supabase.from("likes").select("book_id").eq("user_id", user.id);
      if (likes && likes.length > 0) {
        const { data: lb } = await supabase.from("books").select("*").in("id", likes.map((l) => l.book_id));
        if (lb) setLikedBooks(lb);
      }

      // Bookmarked books
      const { data: bookmarks } = await supabase.from("bookmarks").select("book_id").eq("user_id", user.id);
      if (bookmarks && bookmarks.length > 0) {
        const { data: sb } = await supabase.from("books").select("*").in("id", bookmarks.map((b) => b.book_id));
        if (sb) setSavedBooks(sb);
      }
  ...
  const currentBooks = activeTab === 0 ? publishedBooks : activeTab === 1 ? draftBooks : activeTab === 2 ? likedBooks : savedBooks;

  return (
    <Layout>
      <div className="py-6">
        {profile?.background_image_url && (
          <div className="h-32 -mt-6 mb-4 overflow-hidden">
        ...
        <div className="px-4 mt-4 flex gap-2">
          <Button variant="outline" className="flex-1 font-body text-sm" onClick={() => navigate("/settings")}>Edit Profile</Button>
        ...
        <div className="grid grid-cols-2 gap-2 p-4">
          {currentBooks.length === 0 ? (
            <div className="col-span-2 text-center py-12 text-muted-foreground font-body text-sm">
              {activeTab === 0 ? "No published books yet" : activeTab === 1 ? "No drafts yet" : activeTab === 2 ? "No liked books" : "No saved books"}
            </div>
          ) : (
            currentBooks.map((book) => (
              <button
                key={book.id}
                onClick={() => {
                  if (activeTab === 1) navigate(`/write?book=${book.id}`);
