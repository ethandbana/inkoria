...
import type { WebRTCCall } from "@/hooks/useWebRTC";

interface WebRTCIncomingCallProps {
  call: WebRTCCall;
  onAccept: () => void;
  onDecline: () => void;
}

const WebRTCIncomingCall = ({ call, onAccept, onDecline }: WebRTCIncomingCallProps) => {
  ...
  const initial = (call.partnerName || "?").charAt(0).toUpperCase();
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="flex flex-col items-center gap-6 p-8 rounded-3xl bg-card/90 backdrop-blur-xl shadow-2xl border border-border max-w-xs w-full mx-4">
        <div className="relative">
          <div
            className={`absolute inset-0 rounded-full bg-green-500/30 transition-transform duration-1000 ${
              pulse ? "scale-150 opacity-0" : "scale-100 opacity-50"
            }`}
          />
          <div
            className={`absolute inset-0 rounded-full bg-green-500/20 transition-transform duration-1000 delay-300 ${
              pulse ? "scale-[1.8] opacity-0" : "scale-100 opacity-30"
            }`}
          />
          {call.partnerAvatar ? (
            <img
              ...
              alt=""
              className="w-24 h-24 rounded-full object-cover relative z-10 border-4 border-green-500"
            />
          ) : (
        ...
        <div className="text-center space-y-1">
          <p className="text-lg font-semibold text-foreground">{call.partnerName}</p>
...
export default WebRTCIncomingCall;
